...
**1. 저장소에 저장** — `thinchat` 명령이 0600 저장소(`~/.config/thinchat/credentials.json`)에
기록하고, 이 저장소는 **세션이 끝나도 유지**됩니다. 그래서 한 번만 설정하면 다시 export할 필요
없이(셸 프로파일에 키를 남기지 않고) 모든 세션이 찾습니다; 이 공유 저장소가 이 도구가 더해 주는
부분입니다. 값 전체는 절대 출력되지 않습니다(`set`은 에코 없이 입력받고, `get`은 양 끝만 남기고
마스킹):
